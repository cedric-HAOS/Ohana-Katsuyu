...
def test_read_latest_release_accepts_only_the_official_stable_release(
    ...
    assert requests[0][0].get_header("User-agent") == "Ohana-Katsuyu/0.2.0"
...
def test_refresh_is_cached_for_24_hours_and_preserves_worker_state(
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: (
            ...
            or StableRelease(
                "0.3.0",
                "https://github.com/cedric-HAOS/Ohana-Katsuyu/releases/tag/v0.3.0",
            )
        ),
    )
    ...
    assert second == first
    assert calls == [True]
    assert "mise à jour 0.3.0 disponible" in tooltip(first)
